...
impl CollisionResolver {
	...
	pub fn resolve_weighted(
		...
		match a.intersection(b).is_some() {
			false => None,
			true => {
				let mult_a = 1.0 - weight;
				let mult_b = weight;
				let ow = self.overlap.width;
				let oh = self.overlap.height;
				if oh > ow {
					...
					Some((Vector2f::new(offset_a, 0.0), Vector2f::new(offset_b, 0.0)))
				} else {
					...
					Some((Vector2f::new(0.0, offset_a), Vector2f::new(0.0, offset_b)))
				}
			}
		}
	}
