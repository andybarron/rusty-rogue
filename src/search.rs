...
impl AStarSearch {
    ...
    // fn h_zero(_a: &GraphNode, _b: &GraphNode) -> f32 {
    // 	0.0
    // }
    ...
    fn build_node_path(
        &self,
        came_from: &HashMap<GraphNode, GraphNode>,
        current_node: &GraphNode,
    ) -> Vec<GraphNode> {
        if came_from.contains_key(current_node) {
            let mut path = self.build_node_path(
                came_from,
                came_from.get(current_node).expect("SHOULD have been found"),
            );
...
impl SearchStrategy for AStarSearch {
    fn solve(
        ...
        let start_node = graph
            .find_node_at_tuple(start)
            .expect("ERROR: Could not find start node :-(");
        let end_node = graph
            .find_node_at_tuple(end)
            .expect("ERROR: Could not find end node :-(");
        ...
        f_score.insert(
            start_node,
            g_score.get(&start_node).expect("Start not in g_score")
                + self.h(&start_node, &end_node),
        );
        ...
        while !open.is_empty() {
            ...
            for node in open.iter() {
                let current_f = f_score
                    .get(node)
                    .expect("No expected f score for node")
                    .clone();
                ...
                .iter()
            {
                    ...
                    || tentative_g < *g_score.get(neighbor_ref).expect("NO g for neighbor")
                {
                    ...
                    f_score.insert(
                        neighbor_ref.clone(),
                        g_score.get(neighbor_ref).expect("NO g for neighbor")
