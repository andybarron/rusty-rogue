...
pub trait Screen {
	...
	fn event(&mut self, game: &mut Game, window: &mut RenderWindow, event: Event) -> bool {
		false
	}
	fn key_press(&mut self, game: &mut Game, window: &mut RenderWindow, key: Key) -> bool {
		false
	}
	fn key_release(&mut self, game: &mut Game, window: &mut RenderWindow, key: Key) -> bool {
		false
	}
}
