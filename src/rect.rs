    ...
    T: Num + Copy,
{
    pub fn new(x: T, y: T, w: T, h: T) -> Self {
        Rectangle { data: [x, y, w, h] }
    }
    pub fn from_bounds(min_x: T, min_y: T, max_x: T, max_y: T) -> Self {
        Rectangle {
            data: [min_x, min_y, max_x - min_x, max_y - min_y],
        }
    }
    pub fn from_vectors(min: Vector2<T>, max: Vector2<T>) -> Self {
        Self::from_bounds(min.x, min.y, max.x, max.y)
    }
    pub fn translated(&self, amt: Vector2<T>) -> Self {
        Self::from_vectors(self.min() + amt, self.max() + amt)
    }
    pub fn translated2f(&self, x: T, y: T) -> Self {
        self.translated(Vector2::new(x, y))
    }
    pub fn min(&self) -> Vector2<T> {
        Vector2::new(self.data[0], self.data[1])
    }
    pub fn max(&self) -> Vector2<T> {
        self.min() + Vector2::new(self.data[2], self.data[3])
    }
    pub fn x(&self) -> T {
        self.data[0]
    }
...
impl Rectangle<float> {
    ...
    pub fn center(&self) -> Vector2<float> {
...
impl<T> Into<[T; 4]> for Rectangle<T>
where
    T: Num + Copy,
{
    fn into(self) -> [T; 4] {
